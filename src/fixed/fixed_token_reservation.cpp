...
TokenReservationResult FixedPageManager::reserveTokens(
    RequestId request_id,
    std::uint32_t expected_committed_tokens,
    std::uint32_t token_count)
{
    ...
    std::size_t const maximum_new_pages =
        (static_cast<std::size_t>(token_count) + tokens_per_page_ - 1)
        / tokens_per_page_ + 1;
    ...
    while (remaining_tokens != 0) {
        if (active_entry_index == kNoActiveEntry) {
            PageHandle target = PageHandle::invalid();
            KvCacheError const allocation_error = allocateStagedLocked(0, target);
            if (allocation_error != KvCacheError::None) {
        ...
        std::uint32_t const appended = std::min(
            remaining_tokens,
            static_cast<std::uint32_t>(tokens_per_page_ - active.valid_tokens)
        );
    ...
    if (!insertion.second
        ...
        auto const inserted = token_reservations_.find(reservation_id);
        if (inserted != token_reservations_.end()) {
            rollbackStagedPagesLocked(inserted->second.staged_pages);
            token_reservations_.erase(inserted);
        } else {
            rollbackStagedPagesLocked(staged_pages);
        }
        result.error = KvCacheError::InternalInvariantViolation;
